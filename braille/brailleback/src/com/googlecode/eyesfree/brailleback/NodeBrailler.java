...
public class NodeBrailler {
    /**
     * Height difference between node rectangles under which
     * they are put on the braille display at the same time if adjacent
     * and vertically overlapping.
     */
    private static final int HEIGHT_DIFF_THRESHOLD = 4;
    private final Context mContext;
    private final String TAG="NodeBrailler";
    private final BrailleRuleRepository mRuleRepository;
    private final SelfBrailleManager mSelfBrailleManager;
    public boolean separatorNeeded=false;
    ...
    public DisplayManager.Content brailleNode(
        ...
        Log.d(TAG,"brailleNode called...");
        Log.d(TAG,"Node info : "+node.getPackageName() +" #Text# "+node.getText()+" #SourceView# "+node.getViewIdResourceName());
        DisplayManager.Content content =
            mSelfBrailleManager.contentForNode(node);
        if (content == null) {
            //code added by krittam to find default launcher package name
            ...
            Log.d(TAG,"current home package is "+node.getPackageName());
            if(currentHomePackage.equals(node.getPackageName()))
            {
                Log.d(TAG,"Node detected as menu item");
            ...
            separatorNeeded=false;// For subsequent selections
        ...
        Log.d(TAG,"content sent as "+content.getText());
        return content;
    }
    ...
    private void formatSubtree(AccessibilityNodeInfoCompat node,
        ...
        Log.d(TAG,"formatSubTree called...");
        ...
        if (!TextUtils.isEmpty(subtreeResult)) {
            ...
            if(separatorNeeded)
            {
                StringUtils.appendWithSeparators(result, subtreeResult);
                Log.d(TAG,"used # as seperator");
            }
            else
            {
    ...
    /**
     * Finds the display extent for {@code node}, putting the
     * result in {@code outLeft} and {@code outRight}.
     * The display extent is the first and last node that will
     * contribute to the content of the display when {@code node}
     * has accessibility focus.
     */
    public void findDisplayExtentFromNode(AccessibilityNodeInfoCompat node,
    ...
    /**
     * Finds the list of nodes that should be formatted and put on the braille
     * display together with {@code node}.  The result, including a
     * copy of {@code node} itself, is appended to {@code outResult}.
     */
    private void findNodesToFormat(AccessibilityNodeInfoCompat node,
            ArrayList<AccessibilityNodeInfoCompat> outResult) {
    ...
    /**
     * Finds the range of siblings adjacent to {@code startFrom} that
     * overlap {@code rect} vertically.  Sets {@code outLeft} and
     * {@code outRight} to the inclusive range that overlaps.
     * If there are no overlapping siblings in one of the direction, the
     * corresponding parameter will be set to {@code startFrom}.
     * Returns {@code true} if the range covers all siblings of
     * {@code startFrom}, {@code false} otherwise.
     */
    private boolean findOverlappingRange(AccessibilityNodeInfoCompat startFrom,
            Rect rect,
            AccessibilityNodeInfoRef outLeft,
            AccessibilityNodeInfoRef outRight) {
        Rect tmpRect = new Rect();
        boolean ret = true;
        AccessibilityNodeInfoRef left =
                AccessibilityNodeInfoRef.unOwned(startFrom);
        outLeft.reset(left);
        ...
        AccessibilityNodeInfoRef right =
                AccessibilityNodeInfoRef.unOwned(startFrom);
    ...
    /**
     * Returns {@code true} if the two rectangles should be considered to be
     * on the same 'line' for the purposes of putting horizontally adjacent
     * content on the braille display at the same time.
     */
    private static boolean shouldPutOnSameLine(Rect a, Rect b) {
        ...
        if (ret) {
            ...
            if (aHeight > HEIGHT_DIFF_THRESHOLD * bHeight
                    || bHeight > HEIGHT_DIFF_THRESHOLD * aHeight) {
                ret = false;
            }
        }
        return ret;
