...
public class DisplayManager
    ...
    private static final String TAG_PARENT = "Display Manager";
    ...
    public static class Content {
        ...
        public static final String TAG = "DISPLAY_MANAGER.CONTENT";
        /**
    ...
    */
    ...
    public DisplayManager(TranslatorManager translatorManager,
        ...
        Log.d(TAG_PARENT,"############ Constructor called..");
    ...
    private boolean markSelection(Spanned spanned) {
        Log.d(TAG_PARENT,"markSelection called..");
    ...
    private void copyOverlaidContent() {
        Log.d(TAG_PARENT,"copyOverlaidContent called..");
    ...
    private void extendContentForCursor() {
        Log.d(TAG_PARENT,"extendContentForCursor called..");
    ...
    private void markFocus(Spanned spanned) {
        Log.d(TAG_PARENT,"markFocus called..");
    ...
    private int fixDisplayPosition(int position) {
        Log.d(TAG_PARENT,"fixDisplayPosition called..");
        int numCells = getNumTextCells();
        Log.d(TAG_PARENT,"numCells set to "+numCells);
        int splitIndex = findSplitPointIndex(position);
        Log.d(TAG_PARENT,"splitIndex set to "+splitIndex);
    ...
    public void onConnectionStateChanged(int state) {
        Log.d(TAG_PARENT,"onconnectionStateChanged called..");
        ...
        Log.d(TAG_PARENT,"issimulateddisplay changed to "+mIsSimulatedDisplay);
    ...
    private class DisplayHandler extends Handler {
        ...
        private void handleSetContent(Content content) {
            Log.d(TAG_PARENT,"handle set content called..");
            Content oldContent = mCurrentContent;
            mCurrentContent = content;
            Log.d(TAG_PARENT,"currentContent set to "+mCurrentContent.mText);
            mCursorPosition = findCursorPosition(content);
            Log.d(TAG_PARENT,"cursor position set to "+mCursorPosition);
    ...
    private boolean blinkNeeded(int rightEdge) {
        Log.d(TAG_PARENT,"blink needed called..");
        if (mBrailleContent == mOverlaidBrailleContent) {
            return false;
        }
        Log.d(TAG_PARENT,"mBrailleContent->");
//        logBrailleContent(mBrailleContent);
        Log.d(TAG_PARENT,"mOverlaidBrailleContent->");
//        logBrailleContent(mOverlaidBrailleContent);
    ...
    private int findLeftSplitLimit() {
        Log.d(TAG_PARENT,"find leftsplit limit called..");
        int index = findSplitPointIndex(mDisplayPosition);
        if (index >= 0) {
            int limit = mSplitPoints.keyAt(index);
            if (limit < mDisplayPosition) {
                Log.d(TAG_PARENT,"leftsplit limit is "+limit);
                return limit;

            }
            if (index > 0) {
                Log.d(TAG_PARENT,"leftsplit limit is "+mSplitPoints.keyAt(index - 1));
    ...
    private int findRightSplitLimit() {
        Log.d(TAG_PARENT,"find rightsplit limit called..");
        int index = findSplitPointIndex(mDisplayPosition) + 1;
        if (index >= mSplitPoints.size()) {
            Log.d(TAG_PARENT,"rightsplit limit is "+mBrailleContent.length);
            return mBrailleContent.length;

        }
        Log.d(TAG_PARENT,"rightsplit limit is "+mSplitPoints.keyAt(index));
        return mSplitPoints.keyAt(index);
    }
    private void logBrailleContent(byte[] toPrint) {
        String logMessage="";
        for (int i = 0; i < toPrint.length; i++) {
            logMessage+=toPrint[i];
        }
        Log.d(TAG_PARENT,logMessage);
    }
}
