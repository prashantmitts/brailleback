 ...
 *
 * Note: this display can connect very quickly. To avoid missing any connection
 * state change events, callers should set any necessary listeners before
 * allowing control to return to the {@link Looper} on the current thread.
 */
public final class OverlayDisplay implements Display,
        SharedPreferences.OnSharedPreferenceChangeListener,
        Display.OnConnectionStateChangeListener, Display.OnInputEventListener,
        BrailleView.OnBrailleCellClickListener,
        BrailleView.OnResizeListener {
    private static String TAG="OverlayDisplay";
    ...
    private BrailleDisplayProperties mSimulatedDisplayProperties =
        new BrailleDisplayProperties(
                0 /* numTextCells */, 0 /* numStatusCells */,
    ...
    public void displayDots(byte[] patterns, CharSequence text,
        ...
        Log.d(TAG, "display Dots called...");
        Log.d(TAG, "text is "+text);
        mBackingDisplay.displayDots(patterns, text, brailleToTextPositions);
        mMainThreadHandler.displayDots(getDisplayProperties(), patterns,
                text, brailleToTextPositions);
    }
    ...
    private void updateFromSharedPreferences(SharedPreferences prefs) {
        mOverlayEnabled = SharedPreferencesUtils.getBooleanPref(prefs,
                mContext.getResources(), R.string.pref_braille_overlay_key,
    ...
    private void resizeDisplay(int newSize) {
        ...
        disconnectSimulatedDisplay();
        mSimulatedDisplayProperties = new BrailleDisplayProperties(
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
        if (mContext.getString(R.string.pref_braille_overlay_key)
                .equals(key)) {
            mDisplayThreadHandler.reportPreferenceChange(prefs);
        }
    }
    ...
    /**
     * Main handler for the overlay display.
     * All UI operations must occur through this handler.
     * To enforce this, the overlay is intentionally private.
     */
    private static class MainThreadHandler
            extends WeakReferenceHandler<OverlayDisplay> {
        ...
        public void displayDots(
                BrailleDisplayProperties displayProperties, byte[] patterns,
                CharSequence text, int[] brailleToTextPositions) {
            Log.d(TAG, "display dots 2 called...");
            Log.d(TAG, "Text is "+text);
            synchronized (this) {
        ...
        private void handleDisplayDots() {
            ...
            Log.d(TAG, "handle display dots called...");
            ...
            Log.d(TAG, "Dots displayed as "+ text);
        }
    ...
    /**
     * Handler which runs on the display thread.
     * This is necessary to handle events from the main thread.
     */
    private static class DisplayThreadHandler
            extends WeakReferenceHandler<OverlayDisplay> {
    ...
    private static class BrailleOverlay extends DraggableOverlay {
                ...
                new View.OnHoverListener() {
                    ...
                    public boolean onHover(View view, MotionEvent event) {
                        MotionEvent touchEvent =
                                MotionEventUtils.convertHoverToTouch(event);
                        try {
        ...
        public BrailleOverlay(Context context, final OverlayDisplay parent) {
            ...
            ImageButton panLeftButton = (ImageButton)
                    findViewById(R.id.pan_left_button);
            ...
            ImageButton panRightButton = (ImageButton)
                    findViewById(R.id.pan_right_button);
